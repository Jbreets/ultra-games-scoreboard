...
if (isset($_POST['submit'])) {
    ...
    // leaving space for event information
    ...
    //inserts missing kit into 
    

    $db_events = pull_events('kit-come-back');

    if (in_array($event, $db_events)) {
        // echo"in the database";
    } else {

    $sql = "INSERT INTO `kit-come-back`
    (`event`, `rep`, `glove_set`, `corner_post`, `charity_bucket`, `stamp`, `table_numbers`, `barrier_covers`, `table_cloths`, `ticket_scanner`, `dress_and_cards`, `sign_ups`, `hs_folder`, `go_pro_num`, `camera`, `pole`, `extension_lead`, `black_wire`, `white_plug`, `bracket`, `other_missing`, season)
    VALUES ('$event', '$rep', '$glove_set', '$corner_post', '$charity_bucket', '$stamp', '$table_numbers', '$barrier_covers', '$table_cloths', '$ticket_scanner', '$dress_cards', '$sign_ups', '$hs_folder', '$go_pro_num', '$camera', '$pole', '$extension', '$black_wire', '$white_plug', '$bracket', '$other_missing', '$season') ";
...
?>
<div class="header">
        <h1 class="form-title">Kit Coming Back</h1>
    </div>
        ...
        <div class="form-grid">
            ...
            <div class="answer">
                <select name="event_type" id="">
                    <option hidden value=""><?php print_event_name();  ?></option>
                </select>
...
-->
            ...
            <div class="question">
                <label for="missing">Any Other missing kit / notes</label>
            </div>
            <div class="answer">
